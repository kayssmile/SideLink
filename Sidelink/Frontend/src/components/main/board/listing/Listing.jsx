...
function Listing() {
  ...
  const [shouldRegisterModal, setShouldRegisterModal] = useState(false);
  ...
  const handleShouldRegisterModalAgree = () => {
    setShouldRegisterModal(false);
  };

  /* 
  If user is logged-in we check if public-profile is loaded otherwise we load publicprofile and save it in store.
  If user is not logged-in we show a modal with info.
  If user is logged in but token is expired we show a modal with info to do fresh login.  
  */
  const handleSelectedService = async service => {
    if (await checkAuth()) {
      ...
      dashboardData.user.email ? dispatch(toggleInfoModal()) : setShouldRegisterModal(true);
    }
  };

  useEffect(() => {
    if (publicProfiles.success && selectedService) {
      dispatch(resetPublicProfilesProcess());
  ...
  return (
    <Box component="article">
            ...
            <TableFooter>
              <TableRow>
                <TablePagination
                  ...
                  colSpan={3}
                  count={searchEngineData?.length || 0}
      ...
      {shouldRegisterModal && (
        <Modal
          modalState={shouldRegisterModal}
          ...
          handleAgree={handleShouldRegisterModalAgree}
          usage="single-btn"
        />
      )}
    </Box>
